...
flowchart TB
  ...
  Redis[(Redis session meta)]
  API --> Redis
  H -. optional tool calls via your wiring .-> SS
```
...
控制面镜像 **`Dockerfile`** 内含 `hermes-agent` 与 `agentscope-runtime`。若启用 AgentScope 池且后端为 Docker，镜像内需可用 Docker 守护进程（常见做法：挂载 **`/var/run/docker.sock`**，仅建议测试环境）。Hermes **terminal.backend docker** 同样需要访问 Docker。
